...
## Systemic / meta findings (05-meta, from run trails)

- **L-012** · A session-wide egress denial (all WebFetch → 403, control host included) is an **infrastructure fault, not an agent-instruction defect** — no instruction edit can restore connectivity, and the correct 05-meta response is zero instruction changes. Do NOT "unblock" the research chain by loosening the primary-source rule to accept blog-blended WebSearch numbers: that weakens the "never copy a number from a blog" integrity rule (a hard-limit violation), so a diagnosis that seems to require it is wrong. Correct handling is already in place: research selects the right cluster, refuses to fabricate, logs BLOCKED, and escalates once via an `agent-blocked` issue — leave it to the human. · Evidence: issue #3 (blocked 01-research 2026-07-22/23, re-confirmed egress 403 on 2026-07-24), run 2026-07-24-meta (D-004). · Status: adopted
