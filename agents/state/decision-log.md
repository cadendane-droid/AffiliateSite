...
## 2026-07-24 · 01-research · 2026-07-24-research · D-004
- **Decision:** Chose the **backpacking-headlamps** cluster (Black Diamond Spot 400, Nitecore NU25 UL, Petzl Actik Core) for the next roundup; brief at `content-pipeline/research/2026-07-24-backpacking-headlamps.md`. Appended 3 registry entries (`black-diamond-spot-400`→rei, `nitecore-nu25-ul`→amazon, `petzl-actik-core`→rei), all `PENDING_AFFILIATE_ID` + manufacturer fallback.
- **Why:** `navigation-safety` has **zero** published articles — the thinnest category in `published-index.json` (shelters/water/cooking have roundups; backpacks/sleep-systems have guides), so category balance is maximized. Headlamps are a pure spec object (weight/lumens/beam/IP/battery), the ideal fit for the "weighed and verified" ledger lens. "best backpacking headlamp" is unused as a primary keyword and is a standing high-volume comparison query (dense 2026 coverage: CleverHiker, Switchback Travel, Treeline Review, The Trek). Runners-up: sleeping pads (lost — sleep-systems already has a guide) and trail runners (lost — fit/sizing, not weight, drives the buy; weaker numbers-first fit). No prior decision-log entry touched any of these categories.
- **Desired result:** Once published + indexed, this article should earn HeavyHiker's first organic impressions in the `navigation-safety` category and at least one `affiliate_click` on a headlamp link within 6–8 weeks of affiliate IDs landing. Review by 2026-09-15.
- **Review:** pending
